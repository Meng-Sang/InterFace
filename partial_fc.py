...
class PartialFC(torch.nn.Module):
    ...
    def forward_std(
        ...
        return logits[index, labels[index].view(-1)], labels[index].view(-1)
...
class PartialFCV2(torch.nn.Module):
    """
    https://arxiv.org/abs/2010.05222
    A distributed sparsely updating variant of the FC layer, named Partial FC (PFC).
    ...
    def __init__(
        ...
        """
        self.is_dyn = is_dyn
        super(PartialFCV2, self).__init__()
        assert (
            distributed.is_initialized()
        ), "must initialize distributed before create this"
        self.rank = distributed.get_rank()
    ...
    def forward(
        ...
        # 从所有显卡获取全部权重
        _weight_embeddings = [
            torch.zeros_like(self.weight_activated)
            for _ in range(self.world_size)
        ]
        _list_embeddings = AllGather(local_embeddings, *_gather_embeddings)
        distributed.all_gather(_gather_labels, local_labels)
        ...
        if self.is_dyn:
            distributed.all_gather(_weight_embeddings, self.weight_activated)

        embeddings = torch.cat(_list_embeddings)
        labels = torch.cat(_gather_labels)
        _weight_embeddings = torch.cat(_weight_embeddings)
        _sample_center_embeddings = _weight_embeddings[labels.view(-1)]
        ...
        with torch.cuda.amp.autocast(self.fp16):
            norm_embeddings = normalize(embeddings)
            norm_sample_center_embeddings = normalize(_sample_center_embeddings)
            norm_weight_activated = normalize(self.weight_activated)
            logits = linear(norm_embeddings, norm_weight_activated)
