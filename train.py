...
from losses import CosFace, ArcFace, InterFace
from partial_fc import PartialFC, PartialFCV2
...
def main(args):
    ...
    if cfg.loss == "arcface":
        margin_loss = ArcFace()
    elif cfg.loss.startswith("InterFace"):
        margin_loss = InterFace(margin=cfg.m, mid=cfg.mid, alpha=cfg.alpha, is_dyn=(cfg.loss[-1] == "d"))
    elif cfg.loss == "cosface":
        margin_loss = CosFace()
    else:
        raise
    if cfg.loss != "arcface":
        module_partial_fc = PartialFCV2(
            margin_loss,
            cfg.embedding_size,
            cfg.num_classes,
            cfg.sample_rate,
    ...
    cfg.milestones = [cfg.num_image // total_batch_size * milestone for milestone in cfg.milestones]
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                if global_step % cfg.verbose == 0 and global_step > 200:
                    ...
                    callback_verification.save_best_bc(backbone, cfg.output)
