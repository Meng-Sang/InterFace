...
class CallBackVerification(object):
    ...
    def save_best_bc(self, backbone, output):
        np_bc = np.array(self.bc)
        bc_sum = np.zeros_like(np_bc)
        for i in range(np_bc.shape[0]):
            index = np.argsort(np_bc[i])
            bc_sum[i][index] = np.array(range(1, np_bc.shape[1] + 1))
        # print(np_bc)
        _sum = bc_sum.sum(axis=1)
        # print(_sum)
        # print(np.max(_sum), _sum[-1])
        if np.max(_sum) == _sum[-1]:
            path_module = os.path.join(output, "best_model.pt")
